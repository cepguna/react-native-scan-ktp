...
const useCameraCapture = () => {
  ...
  const { setFace, setFaceUri } = useCameraStore();
  ...
  const onCaptured = useCallback(
    ...
    [camera, setCamera, setFace, setFaceUri, router]
  );

  const onCapturedJS = Worklets.createRunOnJS(onCaptured);

  return { capture, onCapturedJS, shouldTake, loading };
};
...
const useCropRegion = () => {
  ...
  const adaptCropRegion = useCallback(() => {
    ...
    const desiredRegionHeight = regionWidth / (85.6 / 100);
...
export function CameraFace() {
  ...
  const { capture, onCapturedJS, shouldTake, loading } = useCameraCapture();
  ...
  return (
    <View style={StyleSheet.absoluteFill}>
      ...
      <Container className="absolute bottom-6 left-4 z-50">
        <Button onPress={() => router.replace('/')} label="Back" />
      </Container>
    </View>
  );
}
